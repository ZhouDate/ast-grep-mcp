...
import { normalizeWorkspaceDirectory, resolveWorkspacePaths } from "./workspace-paths.js";
...
const DISABLED_TOOLS_ENV = "AST_GREP_DISABLED_TOOLS";
...
async function executeAstGrepTool(name: string, args: unknown, options: AstGrepMcpOptions): Promise<ToolCallResult> {
  ...
  const workspaceDirectory = normalizeWorkspaceDirectory(options.workspaceDirectory ?? process.env.AST_GREP_WORKSPACE ?? process.cwd());
  if (name === "search") {
